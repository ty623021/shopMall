...
public class AbPullToRefreshView extends LinearLayout {
    ...
    private void initContentAdapterView() {
        ...
        if (nestedScrollView==null&&mAdapterView == null && mScrollView == null) {
            throw new IllegalArgumentException("must contain a AdapterView or ScrollView or NestedScrollView in this layout!");
        }
    }
    ...
    private boolean isRefreshViewScroll(int deltaY) {
        ...
        if (nestedScrollView != null) {
            ...
            if (deltaY > 0 && nestedScrollView.getScrollY() == 0) {
                // 判断是否禁用下拉刷新操作
                if (!mEnablePullRefresh) {
                    return false;
                }
                mPullState = PULL_DOWN_STATE;
                return true;
            } else if (deltaY < 0 && child.getMeasuredHeight() <= getHeight() + nestedScrollView.getScrollY()) {
                // 判断是否禁用上拉加载更多操作
                if (!mEnableLoadMore) {
                    return false;
                }
